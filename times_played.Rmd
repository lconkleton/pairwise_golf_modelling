...
models3 <- lapply(seq_len(min(100, length(windows))), function(i) {
  train_bradley_terry3(windows[[i]], i, reference_player)
})
...
extract_significance <- function(model_summary) {
  ...
  return(times_played = times_played_p_value)
}
```
...
```{r}
significance_values_3 <- lapply(models3, function(model) {
  extract_significance(model)
})

significance_df_3 <- do.call(rbind, significance_values_3)
colnames(significance_df_3) <- "times_played_p_value"

significance_df_3 <- as.data.frame(significance_df_3)

View(significance_df_3)
```

Checking mean of p-values:

```{r}
mean(significance_df_3$times_played_p_value, na.rm = TRUE)
```
...
percentage_signif <- significance_df_3 %>%
...
Function for extracting coefficeint values:
```{r}
...
window_numbers <- seq_len(length(models3))
coef_list <- extract_coefficients(models3, window_numbers, reference_player)
...
final_df_3 <- combined_df %>%
  pivot_wider(names_from = Window, values_from = Coefficient) %>%
  arrange(Player)

# remove leading dots from the 'Player' column in final_df
final_df_3$Player <- gsub("^player", "", final_df_3$Player)
```

## PREDICTIVE ANALYSIS

```{r}
predictive_dataframe_3 <- merge(pairwise_matches, window_summary, 
...
predictive_dataframe_3 <- predictive_dataframe_3 %>%
  left_join(final_df_3 %>% 
...
```{r}
times_played_row <- which(final_df_3$Player == "times_played")

times_played_coeffs <- final_df_3[times_played_row, ] %>%
...
correct_predictions <- predictive_dataframe_3 %>%
...
```{r}
log_loss <- predictive_dataframe_3 %>%
...
```{r}
brier_score <- predictive_dataframe_3 %>%
...
```{r}
accuracy_per_window_3 <- predictive_dataframe_3 %>%
  mutate(predicted_win = ifelse(prob_player1_win > 0.5, 1, 0)) %>%
  ...
  filter(num_matches > 0)# can choose to alter this

ggplot(accuracy_per_window_3, aes(x = window_number, y = accuracy)) +
  geom_line(color = "blue") +
  geom_point(data = subset(accuracy_per_window_3, major == 1), color = "red", size = 2.5) + 
  labs(
...
merged_original <- predictive_dataframe_3 %>%
...
merged_swapped <- predictive_dataframe_3 %>%
...
predictive_odds_data_3 <- merged_original %>%
...
predictive_odds_data_3 <- predictive_odds_data_3 %>%
  mutate(
...
```{r}
dim(predictive_odds_data_3)
```

Calculating Accuracy:

```{r}
correct_predictions <- predictive_odds_data_3 %>%
  mutate(predicted_win = ifelse(prob_player1_win > 0.5, 1, 0)) %>%
  summarize(accuracy = mean(predicted_win == player1_win))
...
```{r}
log_loss <- predictive_odds_data_3 %>%
  mutate(log_loss = - (player1_win * log(prob_player1_win) + (1 - player1_win) * log(1 - prob_player1_win))) %>%
  summarize(mean_log_loss = mean(log_loss))
...
```{r}
brier_score <- predictive_odds_data_3 %>%
  mutate(brier_score = (prob_player1_win - player1_win)^2) %>%
  summarize(mean_brier_score = mean(brier_score))
